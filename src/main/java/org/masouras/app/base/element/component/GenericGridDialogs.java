...
public class GenericGridDialogs {
    ...
    public static <T> void showDeleteDialog(T entity, Consumer<List<T>> deleteAction) {
        ...
        dialog.add(new HorizontalLayout(
                VaadinButtonFactory.createButton("Delete", new Icon(VaadinIcon.TRASH), "Delete",
                        _ -> {
                            deleteAction.accept(List.of(entity));
                            dialog.close();
                        }, ButtonVariant.LUMO_WARNING),
                VaadinButtonFactory.createButton("Cancel", new Icon(VaadinIcon.LEVEL_RIGHT), "Cancel", _ -> dialog.close())
        ));
        dialog.open();
    }
